...
Windows installations of SentinelOne also include an AMSI provider may be considered part of the reputation engine as well.
...
It can also be used without Internet access as it only uses a self-contained ML model, Yara signatures, and other detections that are packaged into its resource sections.
...
CMake will also build a `dfi` Python module to allow for easy access to the API.